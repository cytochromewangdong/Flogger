...
{
    NSDictionary *properties = [NSDictionary dictionaryWithObjectsAndKeys:@"iflogger.com", NSHTTPCookieDomain,@"/",NSHTTPCookiePath, @"cache",NSHTTPCookieName,@"readyGO",NSHTTPCookieValue, nil];
...
- (void)start
{
    ...
    [request setAllHTTPHeaderFields:[SDWebImageDownloader cookieDictionary]];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)aConnection
{
    ...
    {
        NSLog(@"====%@",[[[NSString alloc]initWithData:self.imageData encoding:NSUTF8StringEncoding]autorelease]);
        [self connection:aConnection didFailWithError:nil];
        return;
    }
    self.connection = nil;
//    NSLog(@"connect did finish is %@",self.url);
...
{
//    NSLog(@"connect didFailWithError is %@",self.url);
    [[NSNotificationCenter defaultCenter] postNotificationName:SDWebImageDownloadStopNotification object:nil];
