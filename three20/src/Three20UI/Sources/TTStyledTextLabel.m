...
- (void)touchesEnded:(NSSet*)touches withEvent:(UIEvent*)event {
  ...
  //if (!tableView) {
    if(pureTextNode)
    {
        ...
        if([self.actionDelegate handleTap:nil])
        {
            return;
        }
    }
    if (_highlightedNode) {
        ...
        if ([_highlightedNode isKindOfClass:[TTStyledLinkNode class]]) {
            ...
            [param setObject:[(TTStyledLinkNode*)_highlightedNode URL] forKey:@"url"];
            if([self.actionDelegate handleTap:param])
            {
                [self setHighlightedFrame:nil];
                return;
            }
            ...
            NSMutableDictionary *param = [[[NSMutableDictionary alloc]init]autorelease];
            [param setObject:[(TTStyledButtonNode*)_highlightedNode URL] forKey:@"url"];
